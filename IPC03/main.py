...
# Video feed from IPC03 via RTSP (High Resolution)
...
def checkParkingSpace(imPro):
    ...
    cvzone.putTextRect(im, f'Parking Lot Right : {spaceCounter} from {len(posList)}', (20, 49), scale=3,
                       thickness=3, offset=20, colorR=(0, 0, 0))

    """
    # Show Info Full Screen (High Resolution)
    cvzone.putTextRect(im, f'Parking Lot Right : {spaceCounter}', (430, 150), scale=7,
